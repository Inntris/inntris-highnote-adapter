...
| Adapter failure reaching stand-in  | Failures after authentication return an explicit 2xx decline by default; `stand_in` must be selected deliberately                | Unauthenticated, stale or malformed requests still return non-2xx and reach the stand-in setting            |
| Unmapped card silently approved    | A card with no organisational mandate declines instead of returning 422 and deferring to stand-in                                | Snapshot provisioning gaps become declines, so an incomplete snapshot is visible as declined authorisations |
| Malformed downstream answer        | Protocol violations deny under every failure policy and are separated from availability failures                                 | A customer endpoint can still deny availability by never answering                                          |
...
4. Do not let an adapter failure on an authenticated request become an approval by default.
5. Do not claim that signed captured evidence independently proves a Highnote or card network fact.
6. Do not perform a live metadata lookup in the authorisation hot path.
7. Do not log secrets or raw payment payloads.
8. Do not treat an unpinned embedded public key as external trust.
