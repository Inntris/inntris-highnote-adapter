...
export class Ed25519SigningProvider implements SigningProvider {
  ...
  sign(payload: Uint8Array): Promise<string> {
    return Promise.resolve(
      Buffer.from(nacl.sign.detached(payload, this.#secretKey)).toString("base64url"),
    );
  }
}
...
export function decisionFingerprintPayload(
  ...
  return { ...body, signing: signingIdentity };
}
...
export async function attachDecisionSignature(
  ...
  const signable: SignableDecision = { ...unsigned, decision_fingerprint: fingerprint };
  const signature = await signer.sign(canonicalBytes(signable));
  return InntrisDecisionV1Schema.parse({
    ...signable,
    signing: { ...signable.signing, signature },
  });
}
...
export function publicKeyFingerprint(publicKey: Uint8Array): `sha256:${string}` {
  return sha256Bytes(publicKey);
}

export function buildKeyRegistryEntry(
  signer: SigningProvider,
  options: { notBefore: Date; notAfter?: Date | null; status?: KeyRegistryEntry["status"] },
): KeyRegistryEntry {
