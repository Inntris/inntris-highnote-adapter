import { hashAction, hashCanonical } from "./canonical.js";
import {
...
export function verifyDecision(input: {
  ...
  expectedPaymentRequirementsHash?: string;
}): VerificationResult {
  ...
  if (!checks.fingerprint) addReason(reasonCodes, "FINGERPRINT_MISMATCH");
  ...
  if (key === null) {
    ...
    if (!checks.signature) addReason(reasonCodes, "INVALID_SIGNATURE");
  }

  checks.action_hash = hashAction(action) === decision.action_hash;
  if (!checks.action_hash) addReason(reasonCodes, "ACTION_HASH_MISMATCH");
  checks.decision_binding = bindsSuppliedAction(decision, action);
  if (!checks.decision_binding) addReason(reasonCodes, "DECISION_BINDING_MISMATCH");

  const at = input.at ?? new Date();
  checks.expiry =
    at.getTime() >= Date.parse(decision.issued_at) &&
    at.getTime() < Date.parse(decision.expires_at);
  if (!checks.expiry) addReason(reasonCodes, "DECISION_EXPIRED");

  checks.policy_version =
    input.expectedPolicyVersion === undefined ||
    decision.policy.policy_version === input.expectedPolicyVersion;
  if (!checks.policy_version) addReason(reasonCodes, "POLICY_VERSION_MISMATCH");
  ...
  if (!checks.payment_requirements) addReason(reasonCodes, "PAYMENT_REQUIREMENTS_MISMATCH");
