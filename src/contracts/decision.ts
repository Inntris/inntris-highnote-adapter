...
import { attachDecisionSignature, type SigningProvider } from "./signing.js";
...
export async function createSignedDecision(input: CreateDecisionInput): Promise<InntrisDecisionV1> {
  ...
  const issuedAt = (input.clock ?? systemClock).now();
  ...
  return attachDecisionSignature(
    {
      ...
      policy: { policy_hash: input.policyHash, policy_version: input.policyVersion },
      ...
      signing: { alg: input.signer.algorithm, key_id: input.signer.keyId },
    },
    input.signer,
  );
}
